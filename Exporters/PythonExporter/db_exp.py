...
def parse_sql(sql_fname, sql_params):
    ...
    for row in inrows:
        if (
            (row.strip().startswith("--") == True)
            and (row.find(output_file_tag) < 0)
            and (row.find(validation_tag) < 0)
        ):
            continue
        # replace sql params
        ...
        if row.find(output_file_tag) >= 0:
            output_file = row[
                row.find(output_file_tag) + len(output_file_tag) :
            ].strip()
...
def test_env(database=None, sftp=None):
    ...
    if database != None:  # test db packages
        if database not in db_req_packages:
            err_mess = (
                err_mess + "Invalid database type, use mssql, oracle, or postgres\n"
            )
        else:
            if db_req_packages[database] not in installed_packages:
                err_mess = (
                    err_mess
                    + "Package not installed for database connection: {}\n".format(
                        db_req_packages[database]
                    )
                )
...
clparse = argparse.ArgumentParser(
    description="Export from DB using formatted SQL file, optionally create n3c_cohort table"
)
clparse.add_argument(
    "--extract",
    required=False,
    default=None,
    help='specify the name of the file that contains the extract sql, must meet format spec, including ";" and OUTPUT_FILE:',
)
clparse.add_argument(
    "--config",
    required=True,
    help="specify the name of the configuration ini file, see file configuration_ini_example.txt",
)
clparse.add_argument(
    "--database",
    required=False,
    default=None,
    help="specify database, use values oracle or mssql",
)
clparse.add_argument(
    "--output_dir",
    default=".",
    help="specify the directory the csv files will be exported to.  This directory will be zipped if the zip option is set",
)
clparse.add_argument(
    "--arraysize",
    default="10000",
    help="specify the cursor array size, must be integer. The default is 10000.  This is how many records are fetched at once from DB, if you run into memory issues reduce this number",
)
clparse.add_argument(
    "--phenotype",
    default=None,
    help="specify the name of the file that contains the phenotype sql.  This will create the n3c_cohort table",
)
clparse.add_argument(
    "--zip", default=None, help="create zip of output data files", action="store_true"
)
clparse.add_argument(
    "--sftp",
    default=None,
    help="sftp zip file, setup credentials and server in config file",
    action="store_true",
)
clparse.add_argument(
    "--debug",
    default=None,
    help="debug mode, print sql as it is executed and other helpful information",
    action="store_true",
)
...
sql_params = [
    {
        "tag": "@resultsDatabaseSchema",
        "value": config["site"]["results_database_schema"],
    },
    {"tag": "@cdmDatabaseSchema", "value": config["site"]["cdm_database_schema"]},
    {
        "tag": "@vocabularyDatabaseSchema",
        "value": config["site"]["cdm_database_schema"],
    },
    ...
    {
        "tag": "@daysBetweenSubmissions",
        "value": config["site"]["days_between_submissions"],
    },
...
if sql_fname != None:
    ...
    if not os.path.exists(datafiles_dir):
        print(
            "ERROR: export path not found {}.  You may need to create a 'DATAFILES' subdirectory under your output directory, also may need to specify --output on command line\n".format(
                datafiles_dir
            )
        )
        exit(-1)
    ...
    for exp in exports:
        ...
        csvwriter = csv.writer(
            outf, delimiter="|", quotechar='"', quoting=csv.QUOTE_ALL
        )
...
# ZIP #
zip_prefix = (
    config["site"]["site_abbrev"]
    + "_"
    + config["site"]["cdm_name"].lower()
    + "_"
